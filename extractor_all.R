...
#  – Returns a tibble: number | context_sentence | source_link | full_endnote
# --------------------------------------------------------------------------
...
extract_note_hyperlinks <- function(notes, ns_notes, rels_map) {
  urls <- character(0)
...
extract_pages <- function(segment) {
  # if (is.null(segment) || is.na(segment) || !nzchar(segment)) return (NA_character_)
...
full_extractor <- function(docx_path) {
  ...
  # d) Build initial endnotes and page reference dfs----------------------------
  endnotes_df <- tibble(
    number  = ids,
